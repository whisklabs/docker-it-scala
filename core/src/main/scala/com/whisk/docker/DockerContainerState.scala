...
class DockerContainerState(spec: DockerContainer) {
  ...
  private def runReadyCheck()(implicit docker: DockerCommandExecutor, ec: ExecutionContext): Future[Boolean] =
    _isReady.init(
      (for {
        r <- isRunning() if r
  ...
  def getPorts()(implicit docker: DockerCommandExecutor, ec: ExecutionContext): Future[Map[Int, Int]] = {
    def portsFuture: Future[Map[Int, Int]] = getRunningContainer().flatMap {
      ...
      case Some(c) =>
        val ports: Map[Int, Int] = c.ports.collect { case (exposedPort, Seq(binding, _*)) =>
  ...
  def remove(force: Boolean = true, removeVolumes: Boolean = true)(implicit docker: DockerCommandExecutor, ec: ExecutionContext): Future[Unit] =
